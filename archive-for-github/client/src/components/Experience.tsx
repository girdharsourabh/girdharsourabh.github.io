        ...
        <div className="relative max-w-4xl mx-auto">
          ...
          {isLoading || !experiences ? (
            ...
            experiences.map((experience, index) => (
              <div key={experience.id} className={`relative ${index !== experiences.length - 1 ? 'mb-16' : ''}`}>
                <div className="flex flex-col md:flex-row items-start">
                  {/* Timeline dot */}
                  <div className="absolute left-0 md:left-1/2 ml-4 md:ml-0 w-4 h-4 rounded-full bg-primary transform -translate-y-1/2 md:-translate-x-1/2"></div>
                  ...
                  <div className="hidden md:block w-1/2 pr-12 text-right">
                    <span className="text-gray-500 font-medium">{experience.date_range}</span>
                  </div>
                  ...
                  <div className="pl-10 md:pl-0 md:w-1/2 md:pl-12">
                    ...
                    <h4 className="text-gray-700 font-medium mb-3">{experience.company}</h4>
                    <p className="text-gray-600 mb-4">{experience.description}</p>
                    <div className="flex flex-wrap gap-2">
                      {experience.technologies.map((tech, i) => (
                        <Badge key={i} variant="secondary" className="bg-gray-100 text-gray-700 rounded-full">
                          {tech}
                        </Badge>
                      ))}
                    </div>
