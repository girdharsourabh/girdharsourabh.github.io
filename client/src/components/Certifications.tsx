...
export default function CertificationsSection({ certifications, isLoading }: CertificationsProps) {
  ...
  const renderCertificationGrid = (certs: Certification[]) => {
    return (
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {certs.map((cert) => (
          <Card key={cert.id} className="h-full hover:shadow-md transition-shadow">
            <CardHeader className="space-y-1">
            ...
            <CardContent className="mt-1">
              <p className="text-sm text-gray-500 mb-3">Issued: {cert.issue_date}</p>
              {cert.credential_id && (
                <p className="text-sm text-gray-500 mb-3">Credential ID: {cert.credential_id}</p>
              )}
              <div className="flex flex-wrap gap-1.5 mt-3">
                {cert.skills && cert.skills.length > 0 && cert.skills.map((skill, index) => (
              ...
              <div className="max-w-3xl mx-auto">
                {groupByYear(certifications).map(({ year, certs }) => (
                  <div key={year} className="mb-10">
                    <h3 className="text-xl font-semibold mb-4 text-primary">{year}</h3>
                    <div className="space-y-4">
                      {certs.map(cert => (
                        <Card key={cert.id} className="hover:shadow-md transition-shadow">
                          ...
                          <CardContent>
                            {cert.credential_id && (
                              <p className="text-sm text-gray-500 mb-2">Credential ID: {cert.credential_id}</p>
                            )}
                            <div className="flex flex-wrap gap-1.5 mt-3">
                              {cert.skills && cert.skills.length > 0 && cert.skills.map((skill, index) => (
